"""
Start a penance streak for a dead character via the Reaper.
"""
...
class ReaperStartPenanceUseCase:
    ...
    async def execute(self, character_id: uuid.UUID) -> PenanceStreakResponse:
        character = await self._char_repo.find_by_id(character_id)
        if character is None:
            raise CharacterNotFoundException()

        penance = await self._reaper.start_penance(character)
